...
public:
    ...
    constexpr Vector(D x, D y, D z, D w = D(0)) :
        m_x(x),
        m_y(y),
        m_z(z),
        m_w(w)
    ...
    inline const Vector<D, N> operator+ (const Vector<D, N>& other) const {
        return Vector<D, N>(other.m_x + m_x, other.m_y + m_y, other.m_z + m_z, other.m_w + m_w);
    }

    inline const Vector<D, N> operator+ (D other) const {
        return Vector<D, N>(other + m_x, other + m_y, other + m_z, other + m_w);
    }
    ...
    inline const Vector<D, N> operator- (const Vector<D, N>& other) const {
        return Vector<D, N>(m_x - other.m_x, m_y - other.m_y, m_z - other.m_z, m_w - other.m_w);
    }

    /// @brief Unary operator
    inline const Vector<D, N> operator-() const {
        return Vector<D, N>(m_x * -1, m_y * -1, m_z * -1, m_w * -1);
    }

    inline const Vector<D, N> operator-(D other) const {
        return Vector<D, N>(m_x - other, m_y - other, m_z - other, m_w - other);
    }
    ...
    inline friend const Vector<D, N> operator* (D d, const Vector<D, N>& rhs) {
        return Vector<D, N>(rhs.m_x * d, rhs.m_y * d, rhs.m_z * d, rhs.m_w * d);
    }

    inline const Vector<D, N> operator/ (D other) const {
        D factor = D(1) / other;
        return Vector<D, N>(m_x * factor, m_y * factor, m_z * factor, m_w * factor);
    }
    ...
    inline bool operator== (const Vector<D, N>& other) const {
        ...
        return math::areClose(m_x, other.m_x, s_tolerance) && math::areClose(m_y, other.m_y, s_tolerance) && math::areClose(m_z, other.m_z, s_tolerance) && math::areClose(m_w, other.m_w, s_tolerance);
    }
    ...
    inline operator std::vector<D>() const {
        std::vector<D> vec{ m_x, m_y, m_z, m_w };
        return vec;
    }
    ...
    inline std::vector<D> asStdVector() const {
        return std::vector<D>{m_x, m_y, m_z, m_w};
    }
    ...
    inline const DoubleVecType asDouble() const {
        if constexpr (is_double::value) {
            ...
            return DoubleVecType(double(m_x), double(m_y), double(m_z), double(m_w));
        }
    }
    ...
    inline const RealVecType asReal() const {
        if constexpr (is_real::value) {
            ...
            return RealVecType(Real_t(m_x), Real_t(m_y), Real_t(m_z), Real_t(m_w));
        }
    }
    ...
    inline const FloatVecType asFloat() const {
        if constexpr (is_float::value) {
            ...
            return FloatVecType(float(m_x), float(m_y), float(m_z), float(m_w));
        }
    }
...
private:
    ...
    void sortDescending()
    {
        if (m_x < m_y) { swap(&m_x, &m_y); }
        if (m_z < m_w) { swap(&m_z, &m_w); }
        if (m_x < m_z) { swap(&m_x, &m_z); }
        if (m_y < m_w) { swap(&m_y, &m_w); }
        if (m_y < m_z) { swap(&m_y, &m_z); }
    }
    ...
    D m_w{ 0 }; ///< The fourth value stored by the vector
};
