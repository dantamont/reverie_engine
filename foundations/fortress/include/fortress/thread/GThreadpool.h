...
// TODO: Implement expiration timeout
class ThreadPool {
public:
    ...
    uint32_t maxThreadCount() const {
        return m_numThreads.load();
    }

    uint32_t activeThreadCount() const {
        return m_numActiveThreads.load();
    }
    ...
    uint32_t getIndex(std::thread::id threadId) {
        ...
        ->std::future<typename std::invoke_result_t<F, Args...>>
    {
        ...
        std::future<return_type> res = task->get_future();
        {
            ...
            if (m_shutdown.load()) {
                throw std::runtime_error("GThreadPool::addTask: Enqueue on stopped ThreadPool");
            }
...
private:
    ...
    inline void initialize(uint32_t numThreads) {
        if (m_threads.size()) {
    ...
    inline void taskLoop() {
        // Increment active thread count on thread start
        m_numActiveThreads.fetch_add(1);
        ...
        while (true) {
            std::function<void()> task;
            {
                ...
                // Reduce active thread count while waiting
                m_numActiveThreads.fetch_sub(1);
                ...
                // Increment active thread count since no longer waiting
                m_numActiveThreads.fetch_add(1);
    ...
    /// @brief The number of active threads in the pool
    ...
    /// @brief Map of identifiers for each thread and their corresponding IDs in the threads vector
    std::unordered_map<std::thread::id, uint32_t> m_threadIds;
