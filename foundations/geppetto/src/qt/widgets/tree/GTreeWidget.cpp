...
TreeWidgetKeeperWrapper::TreeWidgetKeeperWrapper(QWidget* child)
{
    m_child = child;
    QVBoxLayout* layout1 = new QVBoxLayout(this);
    layout1->setContentsMargins(0, 0, 0, 0);
    layout1->addWidget(m_child);
}

TreeWidgetKeeperWrapper::~TreeWidgetKeeperWrapper()
{
    if (m_child->parent() == this) {
        m_child->hide();
        m_child->setParent(0);
    }
}

QWidget* TreeWidgetKeeperWrapper::child()
{
    return m_child;
}

TreeWidget::TreeWidget(WidgetManager* manager, ActionManager* actionManager, const QString & name, QWidget * parent, uint32_t numColumns) :
