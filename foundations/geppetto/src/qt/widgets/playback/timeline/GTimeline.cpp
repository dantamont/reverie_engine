...
#include "geppetto/qt/widgets/playback/timeline/GTimelineIndicator.h"
#include "geppetto/qt/widgets/playback/timeline/GTimelineTrack.h"
#include "geppetto/qt/widgets/playback/timeline/GTimelineMarker.h"
...
Timeline::Timeline(bool setTickLabels, QObject* _parent):
    QObject(_parent)
{
...
size_t Timeline::addRow(const QString & name, bool refreshTicks)
{
    // Create row header and add to internal list/scene
    ...
    track->setText(name, false, refreshTicks);
...
{
    assert(rowIndex < m_rowHeaders.size() && "Error, timeline does not have enough rows");
...
void Timeline::onResizeRowHeaders(qreal size, bool refreshTicks)
{
...
void Timeline::refreshTickLabels()
{
    ...
    m_scene->setItemIndexMethod(QGraphicsScene::NoIndex); // Speed up adding items
    for (qreal x = left; x < right; x += tickWidth) {
        ...
        // TODO: Set item position before adding to scene, a bit faster
    ...
    m_scene->setItemIndexMethod(QGraphicsScene::BspTreeIndex); // Speed up browsing scene
