#include "GAudioSourceComponentWidget.h"
...
size_t AudioComponentWidget::populateAudioSources()
{
    ...
    for (const auto& resource : m_engine->resourceCache()->topLevelResources()) {
        if (resource->getResourceType() != ResourceType::kAudio) { continue; }

        count++; // Start by incrementing, since 0 index is null resource

        // Get path to audio file and add as item
        QString path = QString(resource->getPath().c_str());
        m_audioSources->addItem(resource->getName().c_str(), path);
        if (audioComponent()->audioHandle()) {
            // If path matches that of the component's current audio handle, set as index
            if (resource->getPath() == audioComponent()->audioHandle()->getPath()) {
                currentIndex = count;
            }
        }
    }
