...
{
    // Format widget sizes
    m_textEdit->setMaximumHeight(0.75f * Viewport::ScreenDPIY());
...
bool RenderLayerWidget::isValidObject(const QJsonObject & object)
{
    ...
    SortingLayer* layer = m_engine->scenario()->settings().renderLayers().getLayer(label);
    if (!layer) {
        return true;
    }
    ...
    bool before)
{
    ...
    m_engine->scenario()->settings().renderLayers().sort();
    repopulate();
}
///////////////////////////////////////////////////////////////////////////////////////////////////
...
void RenderLayerTreeWidget::initializeWidget()
{
    ...
    addAction(kItemSelected,
        ...
        m_engine->scenario()->settings().renderLayers().removeLayer(
            currentContextItem()->sortingLayer()->id(),
            [this](size_t layerId) {
