...
void ServiceManager::connectDeferred(AbstractService* sending_service)
{
    ...
    if (!connected) {
        ...
        createErrorMessage(abstractSender->getName().c_str(), connection.senderSignal().c_str(), abstractReceiver->getName().c_str(), connection.receiverMethod().c_str());
    }
#endif
...
{
    tsl::robin_map<GString, AbstractService*>::iterator iVas;
    iVas = m_serviceMap.find(service->getName());
    if (iVas != m_serviceMap.end()) {
        m_serviceMap.erase(iVas);
    } 
    else {
        QString msg((const char*)("ServiceManager:: removeService: Service " 
            + (GString)service->getName() + " was not found by Service Manager"));
        logError(msg);
    }
}
