...
struct AnimationPlayData {
    AnimationPlayData(const std::shared_ptr<ResourceHandle>& animHandle, const AnimationSettings& settings,
        const AnimationPlaybackMode& mode, const Timer& timer, uint32_t statusFlags);
    ~AnimationPlayData();
...
public:
    ...
    virtual ResourceType getResourceType() const override {
        return ResourceType::kAnimation;
    }
...
class AnimationClip : public Object, public Nameable, public Identifiable, public Serializable {
public:
    //---------------------------------------------------------------------------------------
    /// @name Static
    ...
    virtual const char* namespaceName() const { return "rev::AnimationClip"; }
    /// @}
