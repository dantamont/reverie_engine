...
bool VertexArrayObject::bind()
{
    ...
    if (error) {
        if (!isCreated())
            Logger::LogInfo("Error, VAO not created");
    }
#endif
    ...
    if (error) {
        if (!isCreated()) {
            Logger::LogError("VAO was not created");
        }
        GString id = GString::FromNumber(objectId());
        Logger::LogError("Failed to bind VAO with ID " + id);
    }
#endif
    return !error;
}
...
#ifdef DEBUG_MODE
    if (!created) {
        Logger::LogError("Error creating VAO in OpenGL");
    }
#else
    Q_UNUSED(created);
#endif
    ...
    bool bindThis)
{
    ...
    //unsigned int stride = tupleSize * sizeof(Real_t);
...
unsigned int BufferObject::getTupleSize()
{
    ...
    case BufferAttributeType::kMiscReal:
        return 4;
    default:
        Logger::Throw("Error, type is not handled");
    }
}
