...
void ResourceHandle::PostConstruct(ResourceHandle* handle, const ResourcePostConstructionData& postConstructData)
{
    assert(handle && "Must provide handle");
    ResourceCache& cache = ResourceCache::Instance();

    if (handle->isConstructed()) {
        if (!handle->parent())
            Logger::Throw("Error, reconstructing an already constructed resource");
        else {
    ...
    for (const std::shared_ptr<ResourceHandle>& child : handle->children()) {
        ...
        if (child->isChild()) {
            const ResourcePostConstructionData& childData = cache.m_resourcePostConstructionData[child->getUuid()];
            PostConstruct(child.get(), childData);
        }
    }

    // Post-construct resource
    handle->resource()->postConstruction(postConstructData);
    handle->setConstructed(true);

    // Decrement count of loading objects, making sure not to double decrement
    if (!handle->isChild()) {
        cache.decrementLoadCount();
    }

    // Emit signal that reloaded process (for widgets)
    std::shared_ptr<ResourceHandle> handlePtr = handle->sharedPtr();
    assert(handlePtr && "Error, somehow no handle found");

    /// @fixme Causing slow startup with ResourceTreeWidget
    if (!handlePtr->isHidden()) {
        cache.m_resourceAdded.emitForAll(handlePtr->getUuid());
    }
}
...
void ResourceHandle::postConstruct(const ResourcePostConstructionData& data)
{
    PostConstruct(this, data);
}
