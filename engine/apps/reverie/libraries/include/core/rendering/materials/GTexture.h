...
/// @todo Remove, ripple covers this
...
enum class TexturePropertyFlag {
    //-clamp on | off
...
MAKE_FLAGS(TexturePropertyFlag, TexturePropertyFlags);
MAKE_BITWISE(TexturePropertyFlag);

/// @struct TextureProperties
struct TextureProperties {
    // See: http://paulbourke.net/dataformats/mtl/
    //TextureModelType m_textureModelType = rev::kTextureModelType_None;      // -type (default kTextureModelType_None)
    
    TextureUsageType m_usageType = TextureUsageType::kNone;
    ...
    //    defined in PreView.
    Real_t m_sharpness = 1.0;         // -boost (default 1.0?)
    ...
    //    change the file.
    Real_t m_brightness = static_cast<Real_t>(0.0);        // base_value in -mm option (default 0)
    Real_t m_contrast = static_cast<Real_t>(1.0);          // gain_value in -mm option (default 1)
    ...
    Vector3 m_scale = { static_cast<Real_t>(1.0), static_cast<Real_t>(1.0), static_cast<Real_t>(1.0) }; 
    ...
    TexturePropertyFlags m_flags = TexturePropertyFlag::kBlendHorizontal | TexturePropertyFlag::kBlendVertical;
    ...
    Real_t m_bumpMultiplier = static_cast<Real_t>(1.0);  // -bm (for bump maps only, default 1.0)
...
struct TextureData {
    ...
    std::array<char, MAX_TEX_PATH_SIZE> m_textureFileName;
...
public:
    ...
    static uint32_t MaxDivisibleTextureSize();
    static uint32_t MaxTextureSize();
    static uint32_t MaxNumTextureLayers();
    static uint32_t TextureBoundToActiveUnit();
    ...
    friend void to_json(nlohmann::json& orJson, const Texture& korObject);

    /// @brief Convert from JSON to the given class type
    /// @param korJson The input JSON
    /// @param orObject The object to be obtained from JSON
    friend void from_json(const nlohmann::json& korJson, Texture& orObject);
    ...
    /// @brief Return a QImage of the texture
    // TODO: The easiest way to do this is to set the texture as an FBO attachment, and read the pixels
    // See: https://stackoverflow.com/questions/32070930/copying-a-single-layer-of-a-2d-texture-array-from-gpu-to-cpu
    /*
    GLuint fboId = 0;
    ...
    */
    Image getImage(PixelFormat format, PixelType pixelType, TextureFormat texFormat = TextureFormat::kRGBA8) const;

    /// @brief Return as texture data
    TextureData asTexdata() const;

    /// @brief Get data from OpenGL texture
    void getData(void* outData, PixelFormat format, PixelType pixelType) const;
    ...
    void attach(FrameBuffer& fbo, int attachment, uint32_t depthLayer = 0);
...
protected:
    ...
    /// @details This function takes ownership of the input image's internal buffer
    void initialize(Image& img);

    /// @brief Initialize the texture's image by loading from a file
