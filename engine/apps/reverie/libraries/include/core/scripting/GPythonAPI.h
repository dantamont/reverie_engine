...
class PythonAPI: public SingletonInterface<PythonAPI> {
public:
    /// @name Static
    /// @{
    ...
    void toMatrix(PyObject* incoming, Matrix<T, R, C>& outMatrix) const {
        if (isTuple(incoming)) {
            ...
            if (isList(incoming)) {
                ...
                Logger::Throw("Passed PyObject pointer was not a list or tuple!");
            }
        }
    }
    ...
    py::object toPyDict(const nlohmann::json& json) const;
    ...
    GString cType<GString>(PyObject* o) const {
        ...
        if (s == NULL) {
            printAndClearErrors();
            Logger::Throw("Error, string conversion failed");
        }
