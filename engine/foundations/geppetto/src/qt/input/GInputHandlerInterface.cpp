...
MouseHandler::MouseHandler(InputHandlerInterface* handler):
...
void MouseHandler::onUpdate(double deltaSec)
{    
    Q_UNUSED(deltaSec)
...
void MouseHandler::handleScrollEvent(QWheelEvent * event)
{
    ...
    input.m_angleDelta = Vector2(Constants::DegToRad * event->angleDelta().x() / 8.0,
        Constants::DegToRad * event->angleDelta().y() / 8.0);
...
KeyHandler::KeyHandler(InputHandlerInterface* handler):
    m_inputHandler(handler)
{
}
...
void KeyHandler::onUpdate(double deltaSec)
{
    Q_UNUSED(deltaSec);
    // Move inputs from temporary cache to referenced map
    if (m_inputsCache.size()) {
    ...
    m_mouseHandler(this)
{
    ...
    //s_inputListenerThreads.back() = std::thread(&InputHandlerInterface::inputThreadCallback, this);
}


InputHandlerInterface::InputHandlerInterface(QWidget* widget):
    m_widget(widget),
    m_keyHandler(this),
    m_mouseHandler(this),
    m_holdTime(80),
...
void InputHandlerInterface::update(double deltaSec)
{
    // Collect mouse button presses
    m_mouseHandler.onUpdate(deltaSec);

    // Collect key presses
    m_keyHandler.onUpdate(deltaSec);
}

bool InputHandlerInterface::handleEvent(QInputEvent* inputEvent)
{
    // Dispatch event
    ...
    default:
#ifdef DEBUG_MODE
        assert(false && "Error, input event not handled");
#endif
        handled = false;
        break;
    }
