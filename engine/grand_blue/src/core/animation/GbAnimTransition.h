...
enum class AnimationTransitionType {
    kSmooth, // Clip A and B both play as A fades into B
    kFrozen // Clip A is paused while fading into B
};
...
public:
    ...
    AnimationTransitionType m_transitionType;
...
public:
    ...
    AnimationTransition(AnimationStateMachine* stateMachine, AnimationState* start, AnimationState* end);
    ~AnimationTransition();
    /// @}
    ...
    const AnimationState* start() const { return m_start; }
    const AnimationState* end() const { return m_end; }
...
private:
    ...
    /// @brief The start and end states for the transition
