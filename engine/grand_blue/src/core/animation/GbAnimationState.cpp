...
/////////////////////////////////////////////////////////////////////////////////////////////
bool BaseAnimationState::connectsFrom(const BaseAnimationState * state, AnimationStateMachine* sm, int * outConnectionIndex)
{
    ...
    for (int connectionIndex : m_connections) {
        ...
        //}
        //else {
            if (connection.end()->getUuid() != m_uuid) {
                // Skip if this state isn't the end of connection
                continue;
            }

            const BaseAnimationState* previous = connection.start();
...
/////////////////////////////////////////////////////////////////////////////////////////////
bool BaseAnimationState::connectsTo(const BaseAnimationState * state, AnimationStateMachine* sm, int * outConnectionIndex)
{
    // Iterate over connections to find connected state
    const std::vector<StateConnection>& connections = sm->connections();
    std::vector<BaseAnimationState*> connectedStates;
    for (int connectionIndex : m_connections) {
        ...
        //}
        //else {
            if (connection.start()->getUuid() != m_uuid) {
                // Skip if this state isn't the start of connection
                continue;
            }

            const BaseAnimationState* next = connection.end();
...
void BaseAnimationState::detachConnections(AnimationStateMachine* sm)
{
    ...
    for (int connectionIndex : m_connections) {
        ...
        BaseAnimationState* startState = connection.start();
        BaseAnimationState* endState = connection.end();
...
/////////////////////////////////////////////////////////////////////////////////////////////
QJsonValue BaseAnimationState::asJson() const
{
    QJsonObject object;
...
{
    static size_t clipCount = 0;
    if (animationHandle) {
        Vec::EmplaceBack(m_clips, animationHandle->getName(), animationHandle);
    }
    else {
        Vec::EmplaceBack(m_clips, GString("clip_") + GString::FromNumber(clipCount), nullptr);
        clipCount++;
    }
}
///////////////////////////n////////////////////////////////////////////////////////////////////
...
QJsonValue AnimationState::asJson() const
{
    ...
    //QJsonObject layers;
    //for (AnimationClip* layer : m_layers) {
    //    layers.insert(layer->getName(), layer->asJson());
    //}
    ...
    for (const AnimationClip& clip : m_clips) {
        clips.insert(clip.getName(), clip.asJson());
    }
