#include "GbListenerComponentWidget.h"
...
#include "../../core/events/GbEventListener.h"
...
void ListenerWidget::initializeWidgets()
{
    ...
    if (listenerComponent()->listener()) {
        const std::shared_ptr<PythonClassScript>& script = listenerComponent()->listener()->script();
        if (script) {
            path = script->getPath();
        }
    }
...
{
    outVec.clear();
    QStringList intStrs = m_eventTypes->text().split(",", QString::SkipEmptyParts);
