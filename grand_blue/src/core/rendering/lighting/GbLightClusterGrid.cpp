...
{
    m_clusterGridShaderProgram = camera->m_component->sceneObject()->engine()->resourceCache()->getHandleWithName("light_cluster_grid",
...
void LightClusterGrid::cullLights(MainRenderer & renderer)
{
    ...
    if (!m_camera->frameBuffer().hasSize()) {
        return;
    }
    ...
    std::shared_ptr<ShaderProgram> activeClusterShader = renderer.getActiveClusterShader();
    ...
    m_camera->frameBuffer().bindDepthTexture(0); // Bind depth buffer
    ...
    std::shared_ptr<ShaderProgram> cullingShader = renderer.getLightCullingShader();
...
void LightClusterGrid::onResize()
{
    ...
    View::GLWidget* mainGLWidget = CoreEngine::engines().begin()->second->mainRenderer()->widget();
