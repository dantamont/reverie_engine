#include "GbAnimationNodeWidget.h"
...
#include "../../../core/components/GbAnimationComponent.h"
#include "../../../core/components/GbTransformComponent.h"
#include "../../style/GbFontIcon.h"

#include "../GbNodeViewEditor.h"
#include "../GbNodeViewCanvas.h"
#include "../GbNodeViewBlock.h"
#include "../GbNodeViewScene.h"
#include "../GbNodeViewPort.h"
#include "../GbNodeViewConnection.h"

#include "../../timeline/GbTimeline.h"
#include "../../timeline/GbTimelineTrack.h"
#include "../../timeline/GbTimelineView.h"
#include "../../timeline/GbTimelineScene.h"

#include "../../tree/animation/GbAnimationTreeWidget.h"
#include "../../parameters/animation/GbAnimationMotionWidget.h"
#include "../../parameters/animation/GbAnimationStateWidget.h"
...
NodeViewConnection * AnimationNodeWidget::addConnection(const StateConnection & connection)
{
    ...
    // Create ports
    const BaseAnimationState* endState = connection.end();
    const BaseAnimationState* startState = connection.start();
...
void AnimationNodeWidget::showConnectionMenu(const QPoint& point, NodeViewConnection* connection)
{
    ...
    if (selection == deleteAction)
    {
        ...
        AnimationTransition* transition = new AnimationTransition(controller.stateMachine(), start, end);
...
void AnimationNodeWidget::initializeTimeline()
{
    ...
    for (const std::shared_ptr<ResourceHandle>& child : modelHandle->children()) {
        ...
        if (child->getResourceType() != Resource::ResourceType::kAnimation) {
            continue;
        }

        // Get animation
        std::shared_ptr<Animation> animation = child->resourceAs<Animation>();
        if (!animation) {
            continue;
        }
        size_t rowIndex = m_timeline->addRow(animation->getName());
