...
void BufferUniform::initialize(const ShaderInputInfo & info)
{
    ...
    case ShaderVariableType::kMat4:
        ...
        break;
    default:
        throw("Uniform type is invalid for uniform " + info.name());
        break;
    }
}
...
std::shared_ptr<UBO> UBO::get(const QString & name)
{
    if (!Map::HasKey(s_UBOMap, name)) {
        //std::vector<GString> keys;
        //for (const auto& pair : s_UBOMap) {
...
void UBO::refreshUniform(const BufferUniform & bufferUniform)
{
    ...
    else if (uniform.is<Vector2u>()) {
        glBufferSubData(GL_UNIFORM_BUFFER, bufferUniform.m_offset, bufferUniform.m_alignmentByteSize, &uniform.get<Vector2u>());
    }
    else if (uniform.is<Vector4f>()) {
...
/////////////////////////////////////////////////////////////////////////////////////////////
QJsonValue UBO::asJson(const SerializationContext& context) const {
    QJsonObject object;
