...
void PyInputHandler::PyInitialize(py::module_ & m)
{
    ...
    py::class_<MouseHandler>(m, "MouseHandler")
        .def("__repr__", [](const MouseHandler& m) {return m.asQString().toStdString(); })
    ...
    py::class_<KeyHandler>(m, "KeyHandler")
        .def("__repr__", [](const KeyHandler& k) {return k.asQString().toStdString(); })
