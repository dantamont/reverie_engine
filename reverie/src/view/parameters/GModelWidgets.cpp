...
void CreateMeshWidget::initializeWidgets()
{
    ...
    size_t nameCount = PolygonCache::s_polygonNames.size();
    for (size_t i = 0; i < nameCount; i++) {
        if (i < 2) continue; // Skip rectangle and cube
        GString name = PolygonCache::s_polygonNames[i];
        //GString name = namePair.second;
...
void MeshTreeWidget::repopulate()
{
    ...
    //    if (resourcePair.second->getResourceType() != ResourceType::kMesh) continue;
    ...
    {
        if (resourcePair.second->getResourceType() != ResourceType::kMesh) {
            return;
        }
        addItem(resourcePair.second);
    });
...
{
    if (meshHandle->getResourceType() != ResourceType::kMesh) {
