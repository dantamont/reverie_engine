#include "GAnimationTreeWidget.h"
...
#include "../../parameters/animation/GAnimationClipWidget.h"
...
void AnimationItem::setWidget()
{
    ...
    case AnimationItemType::kMotion: {
        QString name = QString(nameable()->getName().c_str());
        QString txt = text(1);
        if (txt != name) {
...
void AnimationTreeWidget::repopulate()
{
    ...
    case AnimationTreeMode::kStates:
        ...
        for (AnimationTransition* transition : m_controller->stateMachine()->transitions()) {
            addItem(transition);
        }
        break;
...
void AnimationTreeWidget::onItemClicked(QTreeWidgetItem * item, int column)
{
    ...
    const Uuid& objectID = animItem->identifier()->getUuid();
