...
// #include "core/readers/models/GOBJReader.h"
...
{
    m_scene = m_importer.ReadFile(std::string(m_filePath.c_str()),
        //aiProcess_FlipUVs |
    ...
    assert(m_scene && "No scene created");
}

Model* ModelReader::loadModel()
{
    // Instantiate model
    GFile modelFile(m_handle->getPath());

    std::unique_ptr<Model> model_unique = std::make_unique<Model>(Model::ModelFlags());
    m_handle->setName(modelFile.getFileName(true, false));
    ...
    GString extension = modelFile.extension();
    if (extension != "mdl") {
...
void ModelReader::loadAssimp()
{
    ...
    {
        Logger::LogError(GString("ERROR::ASSIMP::") + m_importer.GetErrorString());
    ...
    // TODO: Do this in a different place
    // Serialize model as binary and reference that instead
    //if (!m_handle->getPath().asLower().contains(".mdl")) {
    //    throw std::runtime_error("Error, extension not set for MDL file");
    //}
...
void ModelReader::loadMaterials()
{
    ...
    GFile modelFile(m_filePath);
    GString dir = modelFile.getAbsoluteDirectory();
    for (size_t i = 0; i < m_scene->mNumMaterials; i++) {
        ...
        GString materialPath = dir + GPath::Separator() + mtlName;
        materialPath = GPath::NormalizedPath(materialPath);
...
const Mesh& ModelReader::processMesh(aiMesh * mesh)
{
    ...
    if (mesh->mMaterialIndex >= 0) {
        ...
        m_chunks.back().m_meshHandleIndex = (uint32_t)meshIdx;
        ...
        if (matIter == m_matHandles.end()) {
            Logger::Throw("Error, mat handle not found");
        }
        ...
        Logger::Throw("Error, chunk not created, material not found");
    }
    ...
    if (mesh->mNumBones > 0) {
        ...
        for (uint i = 0; i < mesh->mNumBones; i++) {
            ...
            if (node_it == m_nodeNames.end()) {
                Logger::LogError("Error: Bone " + GString(boneName.c_str()) + " from file" + m_filePath + "not found in scene nodes!\n");
            }
...
MaterialData ModelReader::processMaterial(aiMaterial * material)
{
    ...
    if (AI_SUCCESS != material->Get(AI_MATKEY_COLOR_AMBIENT, ambientColor)) {
        Logger::LogError("Failed to load ambient color property of material");
    }
    else {
    ...
    if (AI_SUCCESS != material->Get(AI_MATKEY_COLOR_DIFFUSE, diffuseColor)) {
        Logger::LogError("Failed to load diffuse color property of material");
    }
    else {
    ...
    if (AI_SUCCESS != material->Get(AI_MATKEY_COLOR_SPECULAR, specularColor)) {
        Logger::LogError("Failed to load specular color property of material");
    }
    else {
    ...
    if (AI_SUCCESS != material->Get(AI_MATKEY_COLOR_EMISSIVE, emissiveColor)) {
        Logger::LogError("Failed to load emissive color property of material");
    }
    else {
    ...
    if (AI_SUCCESS != material->Get(AI_MATKEY_COLOR_TRANSPARENT, transparentColor)) {
        Logger::LogError("Failed to load transparent color property of material");
    }
    else {
    ...
    if (AI_SUCCESS != material->Get(AI_MATKEY_SHININESS, shininess)) {
        Logger::LogError("Failed to load shininess property of material");
    }
    else {
    ...
    if (AI_SUCCESS != material->Get(AI_MATKEY_REFRACTI, ior)) {
        Logger::LogError("Failed to load index of refraction property of material");
    }
    else {
    ...
    if (AI_SUCCESS != material->Get(AI_MATKEY_OPACITY, ior)) {
        Logger::LogError("Failed to load opacity property of material");
    }
    else {
...
Material* ModelReader::createMaterial(const MaterialData & data)
{
    ...
    // NOTE: Might not need this, no longer getting crashes
    // Need to wait until textures are loaded, or else
    // they will not be ready for post-construction
    // TODO: Maybe don't send signal for post-construction from LoadProcess until
    // all child resources are loaded. Maybe use a condition varible or std::future
    // for this
    //bool doneLoading = false;
    //while (!doneLoading) {
    //    // Check child texture status
    //    doneLoading = true;
    //    for (const auto& child : mtl->handle()->children()) {
    //        if (child->isLoading()) {
    //            doneLoading = false;
...
void ModelReader::processTexture(aiMaterial * mat, MaterialData& outMaterial, aiTextureType type)
{
    ...
    GFile texFile(texPathStr);
    GString fileName = texFile.getFileName(true, true);
    texData.setFileName(fileName);
    //texData.m_textureFileDir = FileReader::DirFromPath(texPathStr).toStdString();

    if (GString(texData.m_textureFileName.data()).isEmpty()) {
        Logger::LogWarning("Warning, no texture name found");
    }

    if (mat->GetTextureCount(type) > 1) {
        Logger::LogWarning("Warning, multiple textures of the same type are not handled");
    }

    // Add texture data to material data
    TextureUsageType myType;
    switch (type) {
    case aiTextureType_AMBIENT:
        /// Note, this is actually used for height maps
        /// \see https://github.com/JoeyDeVries/LearnOpenGL/issues/186
        // TODO: Maybe remove ambient
        myType = TextureUsageType::kAmbient;
        break;
    case aiTextureType_DIFFUSE:
        myType = TextureUsageType::kDiffuse;
        break;
    case aiTextureType_NORMALS:
        myType = TextureUsageType::kNormalMap;
        break;
    case aiTextureType_SPECULAR:
        myType = TextureUsageType::kSpecular;
        break;
    case aiTextureType_HEIGHT:
        // Note, this is actually used for normal maps depending on the model type
        /// \see https://github.com/JoeyDeVries/LearnOpenGL/issues/186
        myType = TextureUsageType::kNormalMap;
        break;
    case aiTextureType_SHININESS:
        myType = TextureUsageType::kSpecularHighlight;
        break;
    case aiTextureType_OPACITY:
        myType = TextureUsageType::kOpacity;
        break;
    case aiTextureType_DISPLACEMENT:
        myType = TextureUsageType::kDisplacement;
        break;
    case aiTextureType_REFLECTION:
        myType = TextureUsageType::kReflection;
        break;
    case aiTextureType_LIGHTMAP:
        myType = TextureUsageType::kLightMap;
        break;

    // PBR textures
    case aiTextureType_BASE_COLOR:
        myType = TextureUsageType::kAlbedo_PBR;
        break;
    case aiTextureType_NORMAL_CAMERA:
        myType = TextureUsageType::kBump_PBR;
        break;
    case aiTextureType_EMISSION_COLOR:
        myType = TextureUsageType::kEmissive_PBR;
        break;
    case aiTextureType_METALNESS:
        myType = TextureUsageType::kMetallic_PBR;
        break;
    case aiTextureType_DIFFUSE_ROUGHNESS:
        myType = TextureUsageType::kRoughness_PBR;
        break;
    case aiTextureType_AMBIENT_OCCLUSION:
        myType = TextureUsageType::kAmbientOcclusion_PBR;
        break;
    default:
        Logger::Throw("Texture type not recognized");
        break;
    }

    outMaterial.m_textureData.m_data.resize((size_t)myType + 1);
    outMaterial.m_textureData.m_data[(size_t)myType] = texData;
    outMaterial.m_textureData.m_data[(size_t)myType].m_properties.m_usageType = myType;
}
...
void ModelReader::parseNodeHierarchy(aiNode * assimpNode, uint32_t currentNodeIndex)
{
    ...
    for (uint32_t i = 0; i < assimpNode->mNumChildren; i++) {
        ...
        uint32_t childIndex = skeleton.addChild(currentNodeIndex, aiChildNode->mName.C_Str());
...
void ModelReader::postProcessSkeleton(SkeletonJoint& joint)
{
    ...
    for (uint32_t childIndex : joint.children()) {
