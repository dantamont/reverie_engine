...
bool Model::loadBinary(const GString& filepath)
{
    ...
    auto layoutProtocol = SerializationProtocol<std::vector<VertexAttributeInfo>>(vertexLayout.m_layout);
...
bool Model::saveBinary(const GString& filepath) const
{
    ...
    // Mesh layout
    VertexAttributesLayout vertexLayout;
    auto layoutProtocol = std::make_unique<SerializationProtocol<std::vector<VertexAttributeInfo>>>(vertexLayout.m_layout);
