...
IKNode::IKNode(IKChain* chain, const SkeletonJoint & meshNode):
    m_chain(chain)
{
    // Uuid of IKNode is only to associate with mesh node
    m_uuid = meshNode.getUuid();
...
IKNode * IKChain::getNode(int id) const
{
    return m_nodes[id];
}

IKNode * IKChain::getNode(const SkeletonJoint & joint) const
{
    auto iter = std::find_if(m_nodes.begin(), m_nodes.end(), 
        [joint](IKNode* node) {
        return node->getUuid() == joint.getUuid();
    });
    
    if (iter == m_nodes.end()) {
        Logger::Throw("Error, matching node not found");
    }
...
{
    if (!skeleton.hasRoot()) {
        Logger::Throw("Error, skeleton has no root node");
    }
...
void IKChain::addNode(const SkeletonJoint & meshNode, IKNode* parent)
{
    ...
    // Add to node map
    //if (thisNode->getName().isEmpty()) Logger::Throw("Error, node does not have unique name");
    thisNode->m_nodeID = m_nodes.size();
