...
public:
    ...
    void queueUpdate(const void* data, unsigned long offset, unsigned long sizeInBytes) {
        std::unique_lock lock(m_mutex);
#ifdef DEBUG_MODE
        size_t allowedByteSize = m_buffers[0].byteSize();
        if (allowedByteSize < offset + sizeInBytes) {
            Logger::Throw("Error, assigning bytes out of buffer range");
        }
#endif
    ...
    void queueUpdate(const T& data, unsigned long countOffset) {
        std::unique_lock lock(m_mutex);
        constexpr unsigned long sizeInBytes = (unsigned long)sizeof(T);
        unsigned long byteOffset = countOffset * sizeInBytes;
#ifdef DEBUG_MODE
        unsigned long allowedByteSize = (unsigned long)m_buffers[0].byteSize();
        if (allowedByteSize < byteOffset + sizeInBytes) {
            Logger::Throw("Error, assigning bytes out of buffer range");
        }
#endif
        Vec::EmplaceBack(m_incomingCommands, &data, byteOffset, sizeInBytes);
    }
