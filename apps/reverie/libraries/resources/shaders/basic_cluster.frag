...
// TODO: add fUniqueId and related lines to all color shaders on C++ side to ensure compliance~
// This doesn't have to be automatic, but ShaderProgram
// should have a check for whether or not it uses this location
// in which case the fUniqueId variable will be set
...
layout(binding=0) uniform samplerCubeArray pointShadowMap; // See: https://stackoverflow.com/questions/40469508/opengl-how-to-create-and-bind-cubemap-arrays
...
float pointShadowCalculation(uint lightIndex, float normDotLight)
{
	// Get shadow info
	// See: https://stackoverflow.com/questions/32227283/getting-world-position-from-depth-buffer-value
	ShadowInfo shadow = shadowBuffer.shadows[lightIndex];
    ...
    // float sampledDepth = texture(pointShadowMap, vec4(fragToLight.xyz, mapIndex)).r;
	...
	{
		float closestDepth = texture(pointShadowMap, vec4(fragToLight.xyz + sampleOffsetDirections[i] * diskRadius, mapIndex)).r;
...
vec4 calcLightcolor(uint lightIndex, vec3 unitVectorToCamera)
{
	...
	float ssaoFactor = texture(ssaoTexture, screenCoords).r;
	...
	if(lightType == 0){
		// Point light
		shadowFactor = mapIndex >= 0 ? pointShadowCalculation(lightIndex, normDotLight): 0.0;
	}
	else if(lightType == 1){
		// Directional light
...
void main()
{
	...
	float opacity = texture(material.opacityTexture, uvCoord.st).r;
	...
	fUniqueId = vec4(g_colorId[0] / 255.0f, g_colorId[1] / 255.0f, g_colorId[2] / 255.0f, g_colorId[3] / 255.0f);
