...
void main()
{
	...
	float depth = texture(gDepthTexture, texCoords).r;
	vec3 fragPos = viewPosFromDepth(depth);
	vec3 normal = texture(gNormalTexture, texCoords).xyz;
	vec3 randomVec = normalize(texture(noiseTexture, texCoords * noiseScale).xyz);   // requires repeat
	...
	vec3 noiseTextureSample = normalize(texture(noiseTexture, texCoords).xyz);
